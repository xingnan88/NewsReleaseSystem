...
public class GetNewsServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			{
				request.setAttribute("data1", manager.find("select * from news_title where typeId=1", null,1));
				request.setAttribute("data2", manager.find("select * from news_title where typeId=2", null,1));
				request.setAttribute("data3", manager.find("select * from news_title where typeId=3", null,1));
				request.setAttribute("data4", manager.find("select * from news_title where typeId=4", null,1));
				request.setAttribute("data5", manager.find("select * from news_title where typeId=5", null,1));
				request.setAttribute("data6", manager.find("select * from news_title where typeId=6", null,1));
				request.setAttribute("data7", manager.find("select * from news_title where typeId=7", null,1));
				request.setAttribute("data8", manager.find("select * from news_title where typeId=8", null,1));
			}
